...
async function getBuildDestinationsAsync({
  ...
  const simulators = await _xdl().Simulator.sortDefaultDeviceToBeginningAsync(await (0, _profileMethod().profileMethod)(getSimulatorsAsync)(), osType);
...
async function resolveDeviceAsync(device, {
  ...
  let devices = await getBuildDestinationsAsync({
    osType
  }).catch(() => []);
  spinner.stop();
