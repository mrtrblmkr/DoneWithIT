...
async function ensureSimulatorOpenAsync({
  ...
  if (!udid) {
    udid = await getBestSimulatorAsync({
      osType
    });
  }
...
async function getBestSimulatorAsync({
  ...
  const simulatorOpenedByApp = await isSimulatorBootedAsync(); // This should prevent opening a second simulator in the chance that default
  // simulator doesn't match what the Simulator app would open by default.

  if (simulatorOpenedByApp !== null && simulatorOpenedByApp !== void 0 && simulatorOpenedByApp.udid && (!osType || osType && simulatorOpenedByApp.osType === osType)) {
...
async function isExpoClientInstalledOnSimulatorAsync({
  ...
  return !!(await _internal().SimControl.getContainerPathAsync(udid, EXPO_GO_BUNDLE_IDENTIFIER));
}
...
async function expoVersionOnSimulatorAsync({
  ...
  const localPath = await _internal().SimControl.getContainerPathAsync(udid, EXPO_GO_BUNDLE_IDENTIFIER);
